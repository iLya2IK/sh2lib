...
//
// 11.2023 - additional functionality implemented (iLya2IK)
...
volatile int vread_wb_to = 0;   // timeouts for reading from a non-blocking socket
volatile int vwrite_wb_to = 0;  // timeouts for writing to a non-blocking socket
                                   ...
                                   size_t length)
{
    ...
    if (rv <= 0) {
        if (rv == MBEDTLS_ERR_SSL_WANT_READ || rv == MBEDTLS_ERR_SSL_WANT_WRITE) {
            ...
            vwrite_wb_to++;
            if (vwrite_wb_to > RESET_ON_READ_WB_TIMEOUT) {
                rv = NGHTTP2_ERR_CALLBACK_FAILURE;
                ESP_LOGD(TAG, "write NGHTTP2_ERR_WOULDBLOCK");
                vwrite_wb_to = 0;
            }
        } else {
            rv = NGHTTP2_ERR_CALLBACK_FAILURE;
            ESP_LOGD(TAG, "write NGHTTP2_ERR_CALLBACK_FAILURE");
            vwrite_wb_to = 0;
        }
    } else {
        vwrite_wb_to = 0;
                             ...
                             size_t length, int flags, void *user_data)
{
    ...
    if (rv < 0) {
        if (rv == MBEDTLS_ERR_SSL_WANT_READ || rv == MBEDTLS_ERR_SSL_WANT_WRITE) {
            rv = NGHTTP2_ERR_WOULDBLOCK;
            vread_wb_to++;
            if (vread_wb_to > RESET_ON_READ_WB_TIMEOUT) {
                rv = NGHTTP2_ERR_CALLBACK_FAILURE;
                ESP_LOGD(TAG, "read NGHTTP2_ERR_WOULDBLOCK");
                vread_wb_to = 0;
            }
        } else {
            ESP_LOGD(TAG, "read NGHTTP2_ERR_CALLBACK_FAILURE");
            rv = NGHTTP2_ERR_CALLBACK_FAILURE;
            vread_wb_to = 0;
        }
    } else {
...
static int do_http2_connect(struct sh2lib_handle *hd)
{
    ...
    if (ret != 0) {
        ESP_LOGE(TAG, "[sh2-connect] New http2 session failed with rcode=%d", ret);
    ...
    if (ret != 0) {
        ESP_LOGE(TAG, "[sh2-connect] Submit settings failed with rcode=%d", ret);
        return -1;
    }
    return 0;
}
